...
bool BBox::intersect(const Ray& ray, float &tmin, float &tmax) const {
	...
	tmin = traceraptor_max(tmin, traceraptor_min(t0_y, t1_y));
	tmax = traceraptor_min(tmax, traceraptor_max(t0_y, t1_y));
	if (tmax <= tmin)
		return false;

	float t0_z = (min[2]-ray.o[2])*ray.inv_d[2];
	float t1_z = (max[2]-ray.o[2])*ray.inv_d[2];
	tmin = traceraptor_max(tmin, traceraptor_min(t0_z, t1_z));
	tmax = traceraptor_min(tmax, traceraptor_max(t0_z, t1_z));
	if (tmax <= tmin)
		return false;

	return tmax >= traceraptor_max(0.0f, tmin);
}
