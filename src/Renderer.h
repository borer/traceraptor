...
class Renderer {
    ...
    std::unique_ptr<std::vector<std::shared_ptr<RenderChunk>>> calculate_render_chuncks(int size = 16)
    {
        std::unique_ptr<std::vector<std::shared_ptr<RenderChunk>>> render_chunks = std::make_unique<std::vector<std::shared_ptr<RenderChunk>>>();
...
public:
	...
	void render_scene(const Camera& camera, const Aggregate& world, const std::string& filename, const int number_threads = 1) {
		...
		std::unique_ptr<std::vector<std::shared_ptr<RenderChunk>>> chunks = calculate_render_chuncks(32);
