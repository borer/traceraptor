...
class Material {
public :
	virtual bool scatter(const Ray &r_in, const IntersectionInfo &rec, Sampler sampler, Vec3f &attenuation, Ray& scattered) const = 0;
...
//			cosine = ref_idx * dot(r_in.direction(), rec.normal) / length(r_in.direction());
		...
		if (sampler.random01f() < reflect_prob) {
...
public:
	...
	virtual bool scatter(const Ray &r_in, const IntersectionInfo &rec, const Sampler sampler, Vec3f &attenuation, Ray &scattered) const {
		UNUSED(r_in);
		UNUSED(rec);
		UNUSED(sampler);
		UNUSED(attenuation);
