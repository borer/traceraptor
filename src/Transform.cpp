...
Matrix4x4 Inverse(const Matrix4x4 &m) {
    ...
    for (int i = 0; i < 4; i++) {
            ...
            if (ipiv[j] != 1) {
                for (int k = 0; k < 4; k++) {
                    if (ipiv[k] == 0) {
                        ...
                        Logger::log_error("Singular matrix in MatrixInvert");
                }
            }
        }
        ++ipiv[icol];
        ...
        indxr[i] = irow;
        indxc[i] = icol;
        if (minv[icol][icol] == 0.f) Logger::log_error("Singular matrix in MatrixInvert");
...
Transform LookAt(const Vec3f &pos, const Vec3f &look, const Vec3f &up) {
    ...
    if (length(cross(normalize(up), dir)) == 0) {
    	Logger::log_error(
            "\"up\" vector and viewing direction \"dir\" passed to LookAt are pointing in the same direction.  Using the identity transformation.");
        return Transform();
    }
...
Transform Orthographic(Float zNear, Float zFar) {
    return Scale(1, 1, 1 / (zFar - zNear)) * Translate(Vec3f({0.0f, 0.0f, -zNear}));
}
